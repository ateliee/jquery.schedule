...
(function($) {
	$.fn.timeSchedule = function(options){
		...
		this.getTimeLineNumber = function(top){
			var num = 0;
			var n = 0;
			var tn = Math.ceil(top / setting.timeLineY);
		...
		this.addRow = function(timeline,row){
			...
			if(setting.time_click){
				$timeline.find(".tl").click(function(){
					setting.time_click(jQuery(this).data("time"),timelineData[jQuery(this).data("timeline")]);
				});
			}
		...
		this.resetBarPosition = function(n){
			...
			for(var i=0;i<codes.length;i++){
				...
				$e1.css({top:(h * setting.timeLineY)});
		...
		this.resizeRow = function(n,height){
			...
			$element.find('.sc_data .timeline').eq(n).height((h * setting.timeLineY) - setting.timeLineBorder);
			$element.find('.sc_main .timeline').eq(n).height((h * setting.timeLineY) - setting.timeLineBorder);
